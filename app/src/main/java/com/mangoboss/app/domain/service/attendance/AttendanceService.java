...
public class AttendanceService {
	...
	public void recordClockIn(final ScheduleEntity schedule) {
		...
		if (now.isAfter(schedule.getEndTime())) {
			throw new CustomException(CustomErrorInfo.SCHEDULE_ALREADY_ENDED); // 새로운 에러 코드 정의 필요
		}
	...
	public void validateMethodConsistency(final StoreEntity store, final AttendanceMethod attendanceMethod) {
		final AttendanceMethod current = store.getAttendanceMethod();

		if (current != attendanceMethod) {
