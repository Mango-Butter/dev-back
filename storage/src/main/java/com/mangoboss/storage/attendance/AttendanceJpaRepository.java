...
public interface AttendanceJpaRepository extends JpaRepository<AttendanceEntity, Long> {
    ...
    @Query("""
                SELECT s.staff.id AS staffId,
                       COALESCE(SUM(CASE WHEN a.clockInStatus = 'NORMAL' THEN 1 ELSE 0 END), 0) AS normalCount,
                       COALESCE(SUM(CASE WHEN a.clockInStatus = 'LATE' THEN 1 ELSE 0 END), 0) AS lateCount,
                       COALESCE(SUM(CASE WHEN a.clockInStatus = 'ABSENT' THEN 1 ELSE 0 END), 0) AS absentCount
                FROM ScheduleEntity s
                LEFT JOIN AttendanceEntity a ON a.schedule.id = s.id
